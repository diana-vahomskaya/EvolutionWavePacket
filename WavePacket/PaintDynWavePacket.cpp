...
/** Инициализация GDI+.*/
PaintDynWavePacket::PaintDynWavePacket()
{
...
void PaintDynWavePacket::Exchange(double left, double right, 
...
double PaintDynWavePacket::Trans_X(LPDRAWITEMSTRUCT lpDrawItemStruct, double x)
{
	return (double)(lpDrawItemStruct->rcItem.right) / (double)(_xmax - _xmin)*((x)-_xmin);
}

/** Преобразование у к пиксельному формату.*/
double PaintDynWavePacket::Trans_Y(LPDRAWITEMSTRUCT lpDrawItemStruct, double y)
{
	return -(double)(lpDrawItemStruct->rcItem.bottom) / (double)(_ymax - _ymin)*((y)-_ymax);
}

/** Функция рисования.*/
void PaintDynWavePacket::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	if (!vec_psi_abs.empty())
	{
		...
		double step = 2 * _xmax / vec_psi_abs.size();
		double x1 = _xmin;
		double x2 = _xmin + step;
		for (size_t i = 0; i < vec_psi_abs.size() - 1; i++)
		{
			paint_virtual.DrawLine(&Pen_Gr,
				(Gdiplus::REAL)Trans_X(lpDrawItemStruct, x1),
				(Gdiplus::REAL)Trans_Y(lpDrawItemStruct, vec_psi_abs[i]),
				(Gdiplus::REAL)Trans_X(lpDrawItemStruct, x2),
				(Gdiplus::REAL)Trans_Y(lpDrawItemStruct, vec_psi_abs[i + 1]));
			x1 = x2;
			x2 += step;
		}
	}
